...
def iterate_state(next_state, count_function, cells=4):
    state = 0
        ...
        for j in range(h):
            for i in range(w):
                if state[y][x] == '.':
                    ...
                    next_state[y][x] = 'L' if count_function(
                        state, y, x) >= cells else '#'
                else:
                    next_state[y][x] = '#' if count_function(
                        state, x, y) == 0 else 'L'
    return state


next_state = [[i for i in row] for row in lines]
state = iterate_state(next_state, count_adjacent)
...
next_state = [[i for i in row] for row in lines]
state = iterate_state(next_state, count_adjacent2, cells=5)
print(sum([i == '#' for row in state for i in row]))
