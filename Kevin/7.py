input = """striped orange bags contain 1 vibrant green bag, 5 plaid yellow bags, 1 drab magenta bag.
...
class Bag:
    ...
    def check_key(self, key):
        return key in self.contains

    def what_contains_me(self, iterable):
        return [i for i in iterable if self.name in i.contains]
...
for r in rules:
    ...
    for i in range(4, len(split), 4):
        try:
            content_name = "{} {}".format(split[i+1], split[i+2])
            contents[content_name] = int(split[i])
        except:
            contents = {}
    bag_objects.append(Bag(name, contents))

"""one"""
def recursive_count(count, bags):
    if count > 0:
        for b in bags:
            res = b.what_contains_me(bag_objects)
            for r in res:
                bags.append(r)
    else:
        for b in bag_objects:
            if b.check_key(to_check):
                bags.append(b)
    bag_set = set(bags)
    if len(bag_set) > count:
        return recursive_count(len(bag_set), bags)
    return len(bag_set)
...
sum = 0
def bag_summer(bag, prev_quantity):
    global sum
    depth_sum = 0
    ...
    sum += depth_sum
    for colour, quantity in children.items():
        bag_summer(get_bag_object(colour), quantity * prev_quantity)

bag_summer(get_bag_object(to_check), 1)
print(sum)
